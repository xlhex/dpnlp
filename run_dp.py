...
def train(args, train_dataset, model, tokenizer, config):
    ...
    if args.laplace == 1:
        ...
        dp_delta=1e-3
        gaussian_sigma=calibrateAnalyticGaussianMechanism(args.epsilon, dp_delta, 1, tol = 1.e-12)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.laplace == 1:
                ...
                noise = torch.normal(0, gaussian_sigma, [inputs["input_ids"].shape[0]])
            

            nullify = (torch.rand_like(inputs["attention_mask"].float()) > args.nullification_rate).long()
...
def evaluate(args, model, attacker, processor, tokenizer, config, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, eval_task, processor, tokenizer, evaluate=True)
        ...
        with open(output_eval_file, "w") as writer:
            ...
            for key in sorted(results.keys()):
                ...
                writer.write("%s = %s\n" % (key, str(results[key])))
            logger.info("  %s = %s", "f1", str(f1_score(all_aux_labels, all_aux_preds, average='macro')))
...
def load_and_cache_examples(args, task, processor, tokenizer, evaluate=False):
    ...
    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, *all_aux_labels)
    return dataset
...
def main():
    ...
    # Training
    if args.do_train:
        train_dataset = load_and_cache_examples(args, args.task_name, processor, tokenizer, evaluate=False)
